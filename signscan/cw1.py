...
from signscan.cli import YTrain, label_mapping, signscan, load_data
...
def score_clustering(true_labels, predicted_labels, print_score=False):
    ...
    """
    score = {}
    score["homogeneity"], score["completeness"], score["v_score"] = metrics.cluster.homogeneity_completeness_v_measure(
        column_or_1d(true_labels), predicted_labels)
...
def k_clustering(ctx, sweep_features, sweep_variance, sweep_clusters):
    ...
    best_feature_n = None
    if sweep_features:
        best_feature_n = feature_sweep(features, boolean_labels, labels, seed, save_plot, show_plot, n_features=20)
